...
The schema argument can be a single JSON schema file or a directory
containing sbmd-spec-schema.json.
...
Example:
    ...
    validate_sbmd_specs.py schema/sbmd-spec-schema.json specs/*.sbmd.js
"""
...
def resolve_schema(schema_arg: str) -> Optional[str]:
    """
    Resolve the schema file path.

    If schema_arg is a file, use it directly.
    If schema_arg is a directory, search for sbmd-spec-schema.json.
    """
    if os.path.isfile(schema_arg):
        return schema_arg

    if os.path.isdir(schema_arg):
        for candidate in Path(schema_arg).rglob("sbmd-spec-schema.json"):
...
def validate_sbmd_file(
    ...
    schema_path = resolve_schema(schema_arg)

    if not schema_path:
        print(f"FAIL: {sbmd_file}")
        print(
            f"  Schema: No sbmd-spec-schema.json found in {schema_arg}"
        )
        return 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate SBMD .sbmd.js specification files against "
        "the JSON schema"
    )
    parser.add_argument(
