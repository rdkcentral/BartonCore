...
namespace
{
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().success()");
        EXPECT_EQ(json, R"({"ops":[],"terminal":{"op":"success"}})");
    }

    TEST_F(ResultBuilderTest, ErrorTerminal)
    {
        auto json = EvalAsJson("SbmdUtils.result().error('something failed')");
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().log('hello').success()");
        EXPECT_EQ(json, R"({"ops":[{"op":"log","message":"hello"}],"terminal":{"op":"success"}})");
    }

    TEST_F(ResultBuilderTest, UpdateResourceTwoArgs)
    {
        auto json = EvalAsJson("SbmdUtils.result().dataModel.updateResource('isOn', 'true').success()");
        EXPECT_EQ(json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().dataModel.updateResource('1', 'isOn', 'true').success()");
        EXPECT_EQ(
            json,
    ...
    {
        auto json =
            EvalAsJson("SbmdUtils.result().dataModel.updateResource('1', 'isOn', 'true', {source: 'device'}).success()");
        EXPECT_EQ(
            json,
            R"({"ops":[{"op":"updateResource","endpoint":"1","resource":"isOn","value":"true","options":{"source":"device"}}],"terminal":{"op":"success"}})");
    }

    TEST_F(ResultBuilderTest, SetMetadata)
    {
        auto json = EvalAsJson("SbmdUtils.result().dataModel.setMetadata('1', 'isOn', 'label', 'On/Off').success()");
        EXPECT_EQ(
            json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().storage.setPersistentData('lastState', 'on').success()");
        EXPECT_EQ(
            json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().storage.setTransientData('cache', '42').success()");
        EXPECT_EQ(json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().log('diagnostic').error('failed')");
        EXPECT_EQ(
            json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().device.sendCommand(6, 1)");
        EXPECT_EQ(json, R"({"ops":[],"terminal":{"op":"sendCommand","clusterId":6,"commandId":1}})");
    }

    TEST_F(ResultBuilderTest, SendCommandWithPayload)
    {
        auto json = EvalAsJson("SbmdUtils.result().device.sendCommand(8, 4, 'AQID')");
        EXPECT_EQ(json,
    ...
    {
        auto json = EvalAsJson(
            "SbmdUtils.result().device.sendCommand(257, 0, 'AB==', {timedInvokeTimeoutMs: 10000})");
        EXPECT_EQ(
            json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().device.writeAttribute(3, 0, 'AQID')");
        EXPECT_EQ(
            json,
    ...
    {
        auto json = EvalAsJson("SbmdUtils.result().device.writeAttribute(3, 0, 'AQID', {endpointId: 2})");
        EXPECT_EQ(
            json,
    ...
    TEST_F(ResultBuilderTest, RequestCommand)
    {
        ...
        auto json = EvalAsJson(
            "(function() { var r = SbmdUtils.result().device.requestCommand(257, 0, "
    ...
    {
        auto json = EvalAsJson(
            "(function() { var r = SbmdUtils.result().device.readAttribute(6, 0, { timeoutMs: 3000 });"
            "return { ops: r.ops, terminalOp: r.terminal.op, clusterId: r.terminal.clusterId, "
    ...
    TEST_F(ResultBuilderTest, TerminalCutsOffChaining)
    {
        ...
        EXPECT_TRUE(EvalThrows("SbmdUtils.result().success().log('after')"));
    }

    TEST_F(ResultBuilderTest, StoredBuilderThrowsAfterTerminal)
    {
        // Store builder reference, call terminal, then try to add ops
        EXPECT_TRUE(EvalThrows(
            "(function() { var b = SbmdUtils.result(); b.success(); b.log('after'); })()"));
    }

    TEST_F(ResultBuilderTest, StoredBuilderThrowsAfterTerminalViaDataModel)
    {
        EXPECT_TRUE(EvalThrows(
            "(function() { var b = SbmdUtils.result(); b.success(); b.dataModel.updateResource('x', 'y'); })()"));
    }

    TEST_F(ResultBuilderTest, DoubleTerminalThrows)
    {
        EXPECT_TRUE(EvalThrows(
            "(function() { var b = SbmdUtils.result(); b.success(); b.error('fail'); })()"));
    }
