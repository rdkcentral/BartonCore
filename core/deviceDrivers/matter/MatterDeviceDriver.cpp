...
bool MatterDeviceDriver::ClaimDevice(const DeviceDataCache *deviceDataCache)
{
    ...
    for (chip::EndpointId endpointId : endpointIds)
    {
        ...
        // Check if this endpoint has the Descriptor cluster
        if (!deviceDataCache->EndpointHasServerCluster(endpointId, chip::app::Clusters::Descriptor::Id))
        {
            continue;
        }
        ...
        CHIP_ERROR err = deviceDataCache->GetClusterStateCache()->Get(deviceTypeListPath, reader);
        ...
        // Iterate through the device types on this endpoint
        auto iter = deviceTypeList.begin();
        while (iter.Next())
        {
            auto deviceTypeStruct = iter.GetValue();
            uint16_t deviceTypeId = static_cast<uint16_t>(deviceTypeStruct.deviceType);

            // Check if this device type matches any of our supported types
            for (uint16_t supportedType : deviceTypes)
            {
                if (deviceTypeId == supportedType)
                {
...
bool MatterDeviceDriver::CreateResources(icDevice *device)
{
    ...
    std::string versionStr = deviceCache->GetSoftwareVersionString();
    std::string macAddressStr = deviceCache->GetMacAddress();
    ...
    std::string networkType = deviceCache->GetNetworkType();
...
bool MatterDeviceDriver::RegisterResources(icDevice *device)
{
    ...
    if (deviceCache->GetSerialNumber().has_value())
    {
        serialNumber = strdup(deviceCache->GetSerialNumber().value().c_str());
    }
...
bool MatterDeviceDriver::InitializeDeviceDataCacheIfRequired(const std::string &deviceUuid)
{
    ...
    {
        auto newCache = std::make_unique<DeviceDataCache>(deviceUuid);
