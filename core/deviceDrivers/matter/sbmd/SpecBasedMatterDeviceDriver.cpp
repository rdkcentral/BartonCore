...
bool SpecBasedMatterDeviceDriver::ClaimDevice(DiscoveredDeviceDetails *details)
{
    icDebug();
    ...
    for (auto &entry : details->endpointDescriptorData)
    {
        if (entry.first > 0)
        {
            ...
            {
                if (deviceTypeEntry == spec->matterMeta.deviceType)
                {
                    return true;
                }
            }
...
void SpecBasedMatterDeviceDriver::SynchronizeDevice(std::forward_list<std::promise<bool>> &promises,
...
void SpecBasedMatterDeviceDriver::FetchInitialResourceValues(std::forward_list<std::promise<bool>> &promises,
                                                             const std::string &deviceId,
...
bool SpecBasedMatterDeviceDriver::RegisterResources(icDevice *device, icInitialResourceValues *initialResourceValues)
{
    bool result = true;
                                               ...
                                               const chip::SessionHandle &sessionHandle)
{
    ...
    readContext->mapper = &sbmdResource->mapper;
...
{
    return std::make_unique<SbmdCluster>((SbmdCluster::EventHandler *) this, deviceUuid, endpointId, clusterId);
}
