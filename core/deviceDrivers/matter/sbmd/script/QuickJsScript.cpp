...
namespace barton
{
    ...
    namespace
    {
        ...
        /**
         * Maps SBMD type strings to the JsonToTlv type format strings.
         * @param sbmdType The SBMD type string (e.g., "bool", "uint8", "int16", "string", etc.)
         * @return The JsonToTlv type string (e.g., "BOOL", "UINT", "INT", "STRING", etc.)
         */
        std::string sbmdTypeToJsonTlvType(const std::string &sbmdType)
        {
            // Boolean type
            if (sbmdType == "bool" || sbmdType == "boolean")
            {
                return "BOOL";
            }
            // Unsigned integer types
            if (sbmdType == "uint8" || sbmdType == "uint16" || sbmdType == "uint32" || sbmdType == "uint64" ||
                sbmdType == "enum8" || sbmdType == "enum16" || sbmdType == "bitmap8" || sbmdType == "bitmap16" ||
                sbmdType == "bitmap32" || sbmdType == "bitmap64" || sbmdType == "percent" ||
                sbmdType == "percent100ths" || sbmdType == "epoch-s" || sbmdType == "epoch-us" ||
                sbmdType == "posix-ms" || sbmdType == "elapsed-s" || sbmdType == "utc" || sbmdType == "fabric-idx" ||
                sbmdType == "fabric-id" || sbmdType == "node-id" || sbmdType == "vendor-id" ||
                sbmdType == "devtype-id" || sbmdType == "group-id" || sbmdType == "endpoint-no" ||
                sbmdType == "cluster-id" || sbmdType == "attrib-id" || sbmdType == "field-id" ||
                sbmdType == "event-id" || sbmdType == "command-id" || sbmdType == "action-id" ||
                sbmdType == "trans-id" || sbmdType == "data-ver" || sbmdType == "entry-idx" ||
                sbmdType == "systime-ms" || sbmdType == "systime-us")
            {
                return "UINT";
            }
            // Signed integer types
            if (sbmdType == "int8" || sbmdType == "int16" || sbmdType == "int24" || sbmdType == "int32" ||
                sbmdType == "int40" || sbmdType == "int48" || sbmdType == "int56" || sbmdType == "int64" ||
                sbmdType == "temperature" || sbmdType == "amperage-ma" || sbmdType == "voltage-mv" ||
                sbmdType == "power-mw" || sbmdType == "energy-mwh")
            {
                return "INT";
            }
            // Floating point types
            if (sbmdType == "single" || sbmdType == "float")
            {
                return "FLOAT";
            }
            if (sbmdType == "double")
            ...
            // String types
            if (sbmdType == "string" || sbmdType == "char_string" || sbmdType == "long_char_string")
            {
                return "STRING";
            }
            // Byte string types
            if (sbmdType == "octstr" || sbmdType == "octet_string" || sbmdType == "long_octet_string" ||
                sbmdType == "ipadr" || sbmdType == "ipv4adr" || sbmdType == "ipv6adr" || sbmdType == "ipv6pre" ||
                sbmdType == "hwadr" || sbmdType == "semtag")
            {
                return "BYTES";
            }
            // Struct type
            ...
            // Array/list types
            if (sbmdType == "list" || sbmdType == "array")
            {
                return "ARRAY-?";
            }
            // Null type
            ...
            icLogWarn(LOG_TAG, "Unknown SBMD type '%s', using STRUCT as fallback", sbmdType.c_str());
            return "STRUCT";
        }
         ...
         * @param sbmdType The SBMD type string for type mapping
         * @return The JSON string formatted for JsonToTlv
         */
        std::string formatValueForJsonToTlv(const Json::Value &value, const std::string &sbmdType)
        {
            std::string tlvType = sbmdTypeToJsonTlvType(sbmdType);
        ...
        std::string formatCommandArgsForJsonToTlv(const Json::Value &outputJson, const std::vector<SbmdArgument> &args)
        {
            ...
            if (outputJson.isArray() && args.size() == 1)
            {
                ...
                std::string tlvType = sbmdTypeToJsonTlvType(arg.type);
