    ...
    try
    {
        ...
        if (root["resources"] && root["resources"].IsSequence())
        {
            for (const auto &resourceNode : root["resources"])
            {
                ...
                {
                    // Set resource ID on mapper attributes/commands for root resources
                    if (resource.mapper.hasRead)
                    {
                        if (resource.mapper.readAttribute.has_value())
                        {
                            resource.mapper.readAttribute.value().resourceId = resource.id;
                        }
                        if (resource.mapper.readCommand.has_value())
                        {
                            resource.mapper.readCommand.value().resourceId = resource.id;
                        }
                    }
                    if (resource.mapper.hasWrite)
                    {
                        ...
                        {
                            resource.mapper.writeAttribute.value().resourceId = resource.id;
                        }
                        if (resource.mapper.writeCommand.has_value())
                        {
                            resource.mapper.writeCommand.value().resourceId = resource.id;
                        }
                    }
                    if (resource.mapper.hasExecute)
                    {
                        ...
                        {
                            resource.mapper.executeAttribute.value().resourceId = resource.id;
                        }
                        if (resource.mapper.executeCommand.has_value())
                        {
                            resource.mapper.executeCommand.value().resourceId = resource.id;
                        }
                    }
...
bool SbmdParser::ParseEndpoint(const YAML::Node &node, SbmdEndpoint &endpoint)
{
    ...
    if (node["resources"] && node["resources"].IsSequence())
    {
        for (const auto &resourceNode : node["resources"])
        {
            ...
            if (ParseResource(resourceNode, resource))
            {
                // Set endpoint ID on resource and all mapper attributes and commands
                resource.resourceEndpointId = endpoint.id;
                if (resource.mapper.hasRead)
                {
                    if (resource.mapper.readAttribute.has_value())
                    {
                        resource.mapper.readAttribute.value().resourceEndpointId = endpoint.id;
                        resource.mapper.readAttribute.value().resourceId = resource.id;
                    }
                    if (resource.mapper.readCommand.has_value())
                    {
                        resource.mapper.readCommand.value().resourceEndpointId = endpoint.id;
                        resource.mapper.readCommand.value().resourceId = resource.id;
                    }
                }
                if (resource.mapper.hasWrite)
                {
                    if (resource.mapper.writeAttribute.has_value())
                    {
                        resource.mapper.writeAttribute.value().resourceEndpointId = endpoint.id;
                        resource.mapper.writeAttribute.value().resourceId = resource.id;
                    }
                    if (resource.mapper.writeCommand.has_value())
                    {
                        resource.mapper.writeCommand.value().resourceEndpointId = endpoint.id;
                        resource.mapper.writeCommand.value().resourceId = resource.id;
                    }
                }
                if (resource.mapper.hasExecute)
                {
                    if (resource.mapper.executeAttribute.has_value())
                    {
                        resource.mapper.executeAttribute.value().resourceEndpointId = endpoint.id;
                        resource.mapper.executeAttribute.value().resourceId = resource.id;
                    }
                    if (resource.mapper.executeCommand.has_value())
                    {
                        resource.mapper.executeCommand.value().resourceEndpointId = endpoint.id;
                        resource.mapper.executeCommand.value().resourceId = resource.id;
                    }
                }
...
bool SbmdParser::ParseAttribute(const YAML::Node &node, SbmdAttribute &attribute)
{
    ...
    if (node["useCache"])
    {
        attribute.useCache = node["useCache"].as<bool>();
    }
