...
bool SbmdFactory::RegisterDrivers()
{
    ...
    const std::filesystem::path specsDir = "/home/tlea/BartonCore/core/deviceDrivers/matter/sbmd/specs";

    if (!std::filesystem::exists(specsDir) || !std::filesystem::is_directory(specsDir))
    {
        icWarn("SBMD specs directory does not exist: %s", specsDir.c_str());
        return false;
    }
    ...
    for (const auto& entry : std::filesystem::directory_iterator(specsDir))
    {
        if (entry.is_regular_file() && (entry.path().extension() == ".sbmd"))
        {
            try
            {
                ...
                if (!MatterDriverFactory::Instance().RegisterDriver(driver.release()))
                {
