...
namespace barton
{
                                     ...
                                     const chip::SessionHandle &sessionHandle)
    {
        ...
        {
            icError("Failed to get state value attribute because the cluster state cache expired or was never set");
            return false;
        }
        ...
        if (error == CHIP_NO_ERROR)
        {
            static_cast<BooleanState::EventHandler *>(eventHandler)
                ->StateValueReadComplete(deviceId, value, context);
            result = true;
        }
