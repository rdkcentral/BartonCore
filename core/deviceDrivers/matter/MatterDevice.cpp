...
namespace barton
{
                                                         ...
                                                         const chip::app::ConcreteAttributePath &aPath)
    {
        ...
        if (resourceId != nullptr)
        {
            ...
            observabilitySpanSetAttribute(updateSpan, "device.uuid", device->deviceId.c_str());
            observabilitySpanSetAttribute(updateSpan, "resource.name", resourceId);
            if (resourceEndpointId != nullptr)
            {
                observabilitySpanSetAttribute(updateSpan, "endpoint.profile", resourceEndpointId);
            }
                                                  ...
                                                  const chip::app::StatusIB *apStatus)
    {
        ...
        EventPath eventPath {
            aEventHeader.mPath.mEndpointId, aEventHeader.mPath.mClusterId, aEventHeader.mPath.mEventId};
    ...
    bool
    MatterDevice::GetClusterFeatureMap(chip::EndpointId endpointId, chip::ClusterId clusterId, uint32_t &featureMap)
    {
        chip::TLV::TLVReader reader;
    ...
    void MatterDevice::UpdateCachedFeatureMaps()
    {
        ...
        for (uint32_t clusterId : featureClusters)
        {
            ...
            if (GetEndpointForCluster(clusterId, chipEndpointId))
            {
                ...
                if (GetClusterFeatureMap(chipEndpointId, clusterId, featureMap))
                {
                    ...
                    icDebug("Cached featureMap 0x%x for cluster 0x%x on endpoint %u",
                            featureMap,
                            clusterId,
                            chipEndpointId);
                }
        ...
        icDebug(
            "Updated cached feature maps for device %s (%zu clusters)", deviceId.c_str(), clusterFeatureMaps.size());
    }
    ...
    bool MatterDevice::BindResourceEventInfo(const char *uri, const SbmdEvent &event)
    {
        ...
        EventPath eventPath {
            endpointId, static_cast<chip::ClusterId>(event.clusterId), static_cast<chip::EventId>(event.eventId)};
        ...
        if (binding.type == ResourceBinding::Type::ScriptOnly)
        {
            ...
            else if (result.type == ScriptWriteResult::OperationType::Write)
            {
                ...
                auto writeClient = std::make_unique<chip::app::WriteClient>(
                    const_cast<chip::Messaging::ExchangeManager *>(&exchangeMgr),
                    this,
                ...
                {
                    icError(
                        "Failed to encode preencoded attribute for URI: %s, error: %s", resource->uri, err.AsString());
                    FailOperation(promises);
                    return;
                }
    ...
    {
        icDebug(
            "OnSubscriptionEstablished for device %s, subscription ID: %u", device->deviceId.c_str(), aSubscriptionId);
    ...
    void MatterDevice::OnResponse(const chip::app::WriteClient *apWriteClient,
                                  ...
                                  chip::app::StatusIB attributeStatus)
    {
        icDebug("OnResponse for device %s, endpoint %u, cluster 0x%x, attribute 0x%x, status 0x%x",
                ...
                aPath.mClusterId,
                aPath.mAttributeId,
                static_cast<unsigned int>(attributeStatus.mStatus));
