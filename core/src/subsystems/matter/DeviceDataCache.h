...
public:
    ...
    public:
        ...
        virtual void OnAttributeChanged(DeviceDataCache *cache, const chip::app::ConcreteAttributePath &path) = 0;

        virtual void OnEventData(DeviceDataCache *cache,
                                 const chip::app::EventHeader &aEventHeader,
                                 chip::TLV::TLVReader *apData,
                                 const chip::app::StatusIB *apStatus) = 0;
    };
    ...
    DeviceDataCache(const std::string &deviceUuid) :
        ...
        mOnDeviceConnectionFailureCallback(OnDeviceConnectionFailureCallback, this)
    {
    }
         ...
         * @return The string value, or empty string if not found or on error.
         */
        std::string
        GetStringAttribute(chip::EndpointId endpointId, chip::ClusterId clusterId, chip::AttributeId attributeId) const;

        // Some simple accessors for common Basic Information Cluster attributes
        std::string GetVendorName() const;
        std::string GetProductName() const;
        std::string GetHardwareVersionString() const;
        ...
        /**
         * Get the cluster state cache.
         *
         * @return Pointer to the cluster state cache, or nullptr if not initialized.
         */
        chip::app::ClusterStateCache *GetClusterStateCache() const { return clusterStateCache.get(); }

        /**
         * Set the event handler for this device data cache.
         *
         * @param handler Unique pointer to the event handler. Ownership is transferred to the cache.
         */
        void SetEventHandler(std::unique_ptr<EventHandler> handler) { eventHandler = std::move(handler); }
    ...
    private:
                         ...
                         const chip::app::StatusIB *apStatus) override
        {
            ...
            {
                eventHandler->OnEventData(this, aEventHeader, apData, apStatus);
            }
        }
        ...
        {
            if (eventHandler != nullptr && aStatus.IsSuccess())
            {
                eventHandler->OnAttributeChanged(this, aPath);
            }
        }
