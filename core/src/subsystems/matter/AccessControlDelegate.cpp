 ...
 *
 *    Modified by Comcast to replace Example identifiers with 'zilker' for
 *    production use.
 */
    ...
    */
    ...
    private:
        ...
        static bool IsValid(const Target &target)
        {
            ...
            return ((target.flags & kNone) == 0) && ((target.flags & kAtLeastOne) != 0) &&
                   ((target.flags & kNotAll) != kNotAll) &&
    ...
    public:
                ...
                if (err == CHIP_NO_ERROR)
                {
                    ...
                    if (index != nullptr)
                    {
                        *index = size_t(storage - EntryStorage::acl);
                        if (fabricIndex != nullptr)
                        {
                            EntryStorage::ConvertIndex(
                                *index, *fabricIndex, EntryStorage::ConvertDirection::kAbsoluteToRelative);
                        }
                    }
                }
                return err;
        ...
        CHIP_ERROR Check(const SubjectDescriptor &subjectDescriptor,
