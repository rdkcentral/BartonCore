...
DeviceDataCache::~DeviceDataCache()
{
    ...
    // Schedule cleanup on Matter thread to avoid threading issues
    auto resources = new std::pair<std::unique_ptr<chip::app::ReadClient>, std::unique_ptr<chip::app::ClusterStateCache>>(
        std::move(readClient), std::move(clusterStateCache));

    chip::DeviceLayer::PlatformMgr().ScheduleWork([](intptr_t arg) {
        auto* res = reinterpret_cast<std::pair<std::unique_ptr<chip::app::ReadClient>, std::unique_ptr<chip::app::ClusterStateCache>>*>(arg);
        delete res; // Resources will be cleaned up when unique_ptrs go out of scope
    }, reinterpret_cast<intptr_t>(resources));
}
...
std::future<bool> DeviceDataCache::Start()
{
    ...
    if (startupPromise)
    {
        ...
        auto future = failedPromise->get_future();
        failedPromise->set_value(false);
        return future;
    }
    startupPromise = std::make_unique<std::promise<bool>>();
    auto future = startupPromise->get_future();
    lock.~lock_guard(); // Manually unlock before scheduling work

    chip::DeviceLayer::PlatformMgr().ScheduleWork([](intptr_t arg) {
        auto* self = reinterpret_cast<DeviceDataCache*>(arg);

        if (Matter::GetInstance().GetCommissioner().GetConnectedDevice(barton::Subsystem::Matter::UuidToNodeId(self->deviceUuid),
...
std::string DeviceDataCache::GetStringAttribute(chip::EndpointId endpointId,
                                                chip::ClusterId clusterId,
                                                chip::AttributeId attributeId) const
{
...
std::string DeviceDataCache::GetVendorName() const
{
...
std::string DeviceDataCache::GetProductName() const
{
    return GetStringAttribute(0,
                              chip::app::Clusters::BasicInformation::Id,
...
std::string DeviceDataCache::GetHardwareVersionString() const
{
    return GetStringAttribute(0,
                              chip::app::Clusters::BasicInformation::Id,
...
std::string DeviceDataCache::GetSoftwareVersionString() const
{
    return GetStringAttribute(0,
                              chip::app::Clusters::BasicInformation::Id,
...
std::optional<std::string> DeviceDataCache::GetSerialNumber() const
{
    std::string result = GetStringAttribute(0,
                                            chip::app::Clusters::BasicInformation::Id,
...
std::string DeviceDataCache::GetMacAddress() const
{
...
std::string DeviceDataCache::GetNetworkType() const
{
    auto interfaceInfo = GetInterfaceInfo();
    if (!interfaceInfo.HasValue())
    {
...
chip::Optional<NetworkUtils::NetworkInterfaceInfo> DeviceDataCache::GetInterfaceInfo() const
{
    ...
    CHIP_ERROR error = clusterStateCache->Get<TypeInfo>({chip::kRootEndpointId,
        chip::app::Clusters::GeneralDiagnostics::Id, chip::app::Clusters::GeneralDiagnostics::Attributes::NetworkInterfaces::Id}, value);
    if (error == CHIP_NO_ERROR)
    {
        auto sessionMgr =
            chip::app::InteractionModelEngine::GetInstance()->GetExchangeManager()->GetSessionManager();
        chip::NodeId nodeId = Subsystem::Matter::UuidToNodeId(deviceUuid);
...
std::vector<chip::EndpointId> DeviceDataCache::GetEndpointIds() const
{
    ...
    chip::app::ConcreteAttributePath partsListPath(0, chip::app::Clusters::Descriptor::Id,
...
SubscriptionIntervalSecs DeviceDataCache::CalculateFinalSubscriptionIntervalSecs()
{
    ...
    icDebug("Will request max interval ceiling of %d seconds for the subscription", intervalSecs.maxIntervalCeilingSecs);
...
Json::Value DeviceDataCache::GetEndpointAsJson(chip::EndpointId endpointId)
{
    ...
    clusterStateCache->ForEachCluster(endpointId, [this, endpointId, &endpointJson](chip::ClusterId clusterId) {
        ...
        clusterStateCache->ForEachAttribute(endpointId, clusterId, [this, &clusterJson](const chip::app::ConcreteAttributePath & path) {
...
void DeviceDataCache::OnDeviceConnectionFailure(const chip::ScopedNodeId &peerId,
                                               CHIP_ERROR error)
{
    icDebug();
