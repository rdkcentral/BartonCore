...
namespace barton {
    ...
    public:
        ...
        template <typename T>
        typename std::enable_if<std::is_base_of<BartonCommissionableDataProvider, T>::value, bool>::type
        RegisterBartonCommissionableDataProvider(std::shared_ptr<T> provider)
        {
            g_return_val_if_fail(provider, false);
            g_return_val_if_fail(!commissionableDataProvider, false);

            commissionableDataProvider = std::static_pointer_cast<BartonCommissionableDataProvider>(provider);
            return true;
        }

        std::shared_ptr<BartonCommissionableDataProvider> GetBartonCommissionableDataProvider()
        {
