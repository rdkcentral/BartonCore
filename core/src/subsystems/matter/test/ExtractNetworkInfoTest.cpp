...
namespace
{
    ...
    {
        protected:
            Type CreateInterface(const char * name, bool isOperational, const uint8_t * macAddress, size_t macLen, InterfaceTypeEnum type, const std::vector<ByteSpan> & ipv6Addresses)
            {
                Type iface;
    ...
    {
        uint8_t ipv6Addr1[16] = { 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 };
        std::vector<ByteSpan> ipv6Addresses = { ByteSpan(ipv6Addr1, sizeof(ipv6Addr1)) };
        Type iface = CreateInterface("eth0", false, (const uint8_t *) "\x00\x1A\x2B\x3C\x4D\x5E", 6, EMBER_ZCL_INTERFACE_TYPE_ENUM_ETHERNET, ipv6Addresses);
    ...
    {
        Type iface = CreateInterface("eth0", true, (const uint8_t *) "\x00\x1A\x2B\x3C\x4D\x5E", 6, EMBER_ZCL_INTERFACE_TYPE_ENUM_ETHERNET, {});
    ...
    TEST_F(ExtractOperationalInterfaceInfoTest, MatchingIPv6Address)
    {
        uint8_t ipv6Addr1[16] = { 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 };
        std::vector<ByteSpan> ipv6Addresses = { ByteSpan(ipv6Addr1, sizeof(ipv6Addr1)) };
        Type iface = CreateInterface("eth0", true, (const uint8_t *) "\x00\x1A\x2B\x3C\x4D\x5E", 6, EMBER_ZCL_INTERFACE_TYPE_ENUM_ETHERNET, ipv6Addresses);
