...
static bool isZeroTouchDevice(const char *deviceUuid)
{
    ...
    // devices (a parse failure means this is not a zigbee eui64).
    uint64_t eui64 = 0;
...
bool deviceServiceDeviceFound(DeviceFoundDetails *deviceFoundDetails, bool neverReject)
{
    ...
    if (dd == NULL)
    {
        ...
        else if (isZeroTouchDevice(deviceFoundDetails->deviceUuid))
        {
            // Zero-touch (Zigbee Direct) devices are explicitly permitted to pair
            // out-of-band via the zeroTouchEui64s policy, so they are trusted to
            // pair without a device descriptor (no descriptor bypass required).
            icLogDebug(LOG_TAG,
                       "%s: zero-touch device %s is permitted; allowing device to be paired",
                       __FUNCTION__,
                       deviceFoundDetails->deviceUuid);
        }
        else
