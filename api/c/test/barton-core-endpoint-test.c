...
static void setup(BCoreEndpointTest *test, gconstpointer user_data)
{
    test->endpoint = b_core_endpoint_new();
}

// Teardown function called after each test
static void teardown(BCoreEndpointTest *test, gconstpointer user_data)
{
    g_clear_object(&test->endpoint);
}

// Test case to check if endpoint is created successfully
static void test_endpoint_creation(BCoreEndpointTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->endpoint);
}

// Test case to check setting and getting properties
static void test_property_access(BCoreEndpointTest *test, gconstpointer user_data)
{
    // Set and get properties and check their values
    g_object_set(
        test->endpoint, B_CORE_ENDPOINT_PROPERTY_NAMES[B_CORE_ENDPOINT_PROP_ID], "test-id", NULL);
    gchar *id = NULL;
    g_object_get(
        test->endpoint, B_CORE_ENDPOINT_PROPERTY_NAMES[B_CORE_ENDPOINT_PROP_ID], &id, NULL);
    g_assert_cmpstr(id, ==, "test-id");
    g_free(id);

    g_object_set(
        test->endpoint, B_CORE_ENDPOINT_PROPERTY_NAMES[B_CORE_ENDPOINT_PROP_URI], "test-uri", NULL);
    gchar *uri = NULL;
    g_object_get(
        test->endpoint, B_CORE_ENDPOINT_PROPERTY_NAMES[B_CORE_ENDPOINT_PROP_URI], &uri, NULL);
    ...
    g_object_set(
        test->endpoint, B_CORE_ENDPOINT_PROPERTY_NAMES[B_CORE_ENDPOINT_PROP_ENABLED], TRUE, NULL);
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-endpoint/endpoint-creation",
               BCoreEndpointTest,
               NULL,
               setup,
               test_endpoint_creation,
               teardown);
    g_test_add("/barton-core-endpoint/property-access",
               BCoreEndpointTest,
               NULL,
               setup,
