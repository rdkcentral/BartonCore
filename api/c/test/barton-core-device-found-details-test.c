...
typedef struct
{
    BCoreDeviceFoundDetails *details;
} BCoreDeviceFoundDetailsTest;

// Setup function called before each test
static void setup(BCoreDeviceFoundDetailsTest *test, gconstpointer user_data)
{
    test->details = b_core_device_found_details_new();
}

// Teardown function called after each test
static void teardown(BCoreDeviceFoundDetailsTest *test, gconstpointer user_data)
{
    g_clear_object(&test->details);
}

// Test case to check if device is created successfully
static void test_discovery_details_creation(BCoreDeviceFoundDetailsTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->details);
}

// Test case to check setting and getting properties
static void test_property_access(BCoreDeviceFoundDetailsTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-device-found-details/device-creation",
               BCoreDeviceFoundDetailsTest,
               NULL,
               setup,
               test_discovery_details_creation,
               teardown);
    g_test_add("/barton-core-device-found-details/property-access",
               BCoreDeviceFoundDetailsTest,
               NULL,
               setup,
