...
{
    BCoreStatus *status;
    BCoreStatusEvent *event;
} BCoreStatusTest;

// Setup function called before each test
static void setup(BCoreStatusTest *test, gconstpointer user_data)
{
    test->status = b_core_status_new();
    test->event = b_core_status_event_new();
}

// Teardown function called after each test
static void teardown(BCoreStatusTest *test, gconstpointer user_data)
{
...
static void test_status_creation(BCoreStatusTest *test, gconstpointer user_data)
{
...
static void test_property_access(BCoreStatusTest *test, gconstpointer user_data)
{
    // test device class
    ...
    // test json
    g_object_set(
        test->status, B_CORE_STATUS_PROPERTY_NAMES[B_CORE_STATUS_PROP_JSON], "test-json", NULL);
    gchar *json = NULL;
    g_object_get(test->status, B_CORE_STATUS_PROPERTY_NAMES[B_CORE_STATUS_PROP_JSON], &json, NULL);
...
static void test_event_object(BCoreStatusTest *test, gconstpointer user_data)
{
    BCoreStatus *status = b_core_status_new();
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-status/status-creation",
               BCoreStatusTest,
               NULL,
               setup,
               test_status_creation,
               teardown);
    g_test_add("/barton-core-status/property-access",
               BCoreStatusTest,
               NULL,
               setup,
               ...
               teardown);
    g_test_add(
        "/barton-core-status/event-object", BCoreStatusTest, NULL, setup, test_event_object, teardown);
