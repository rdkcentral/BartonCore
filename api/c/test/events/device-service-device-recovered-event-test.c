...
typedef struct
{
    BDeviceServiceDeviceRecoveredEvent *event;
} BDeviceServiceDeviceRecoveredEventTest;

static void setup(BDeviceServiceDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    test->event = b_device_service_device_recovered_event_new();
}

// Teardown function called after each test
static void teardown(BDeviceServiceDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BDeviceServiceDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BDeviceServiceDeviceRecoveredEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/device-service-device-recovered-event/event-creation",
               BDeviceServiceDeviceRecoveredEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/device-service-device-recovered-event/property-access",
               BDeviceServiceDeviceRecoveredEventTest,
               NULL,
               setup,
