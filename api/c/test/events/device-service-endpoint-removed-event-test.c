...
typedef struct
{
    BDeviceServiceEndpointRemovedEvent *event;
} BDeviceServiceEndpointRemovedEventTest;

static void setup(BDeviceServiceEndpointRemovedEventTest *test, gconstpointer user_data)
{
    test->event = b_device_service_endpoint_removed_event_new();
}

// Teardown function called after each test
static void teardown(BDeviceServiceEndpointRemovedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BDeviceServiceEndpointRemovedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BDeviceServiceEndpointRemovedEventTest *test, gconstpointer user_data)
{
    ...
    gboolean device_removed_test = FALSE;
    g_object_get(test->event,
                 B_DEVICE_SERVICE_ENDPOINT_REMOVED_EVENT_PROPERTY_NAMES
                     [B_DEVICE_SERVICE_ENDPOINT_REMOVED_EVENT_PROP_DEVICE_REMOVED],
                 &device_removed_test,
    ...
    g_assert_cmpint(device_removed_test, ==, device_removed);
}
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/device-service-endpoint-removed-event/event-creation",
               BDeviceServiceEndpointRemovedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/device-service-endpoint-removed-event/property-access",
               BDeviceServiceEndpointRemovedEventTest,
               NULL,
               setup,
