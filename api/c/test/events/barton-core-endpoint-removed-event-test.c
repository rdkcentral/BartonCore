...
typedef struct
{
    BCoreEndpointRemovedEvent *event;
} BCoreEndpointRemovedEventTest;

static void setup(BCoreEndpointRemovedEventTest *test, gconstpointer user_data)
{
    test->event = b_core_endpoint_removed_event_new();
}

// Teardown function called after each test
static void teardown(BCoreEndpointRemovedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BCoreEndpointRemovedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BCoreEndpointRemovedEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-endpoint-removed-event/event-creation",
               BCoreEndpointRemovedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/barton-core-endpoint-removed-event/property-access",
               BCoreEndpointRemovedEventTest,
               NULL,
               setup,
