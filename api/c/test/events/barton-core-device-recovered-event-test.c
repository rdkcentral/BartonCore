...
typedef struct
{
    BCoreDeviceRecoveredEvent *event;
} BCoreDeviceRecoveredEventTest;

static void setup(BCoreDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    test->event = b_core_device_recovered_event_new();
}

// Teardown function called after each test
static void teardown(BCoreDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BCoreDeviceRecoveredEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BCoreDeviceRecoveredEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-device-recovered-event/event-creation",
               BCoreDeviceRecoveredEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/barton-core-device-recovered-event/property-access",
               BCoreDeviceRecoveredEventTest,
               NULL,
               setup,
