...
typedef struct
{
    BDeviceServiceDiscoveryStartedEvent *event;
} BDeviceServiceDiscoveryStartedEventTest;

static void setup(BDeviceServiceDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    test->event = b_device_service_discovery_started_event_new();
}

// Teardown function called after each test
static void teardown(BDeviceServiceDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BDeviceServiceDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BDeviceServiceDiscoveryStartedEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/device-service-discovery-started-event/event-creation",
               BDeviceServiceDiscoveryStartedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/device-service-discovery-started-event/property-access",
               BDeviceServiceDiscoveryStartedEventTest,
               NULL,
               setup,
