...
typedef struct
{
    BCoreDeviceRemovedEvent *event;
} BCoreDeviceRemovedEventTest;

static void setup(BCoreDeviceRemovedEventTest *test, gconstpointer user_data)
{
    test->event = b_core_device_removed_event_new();
}

// Teardown function called after each test
static void teardown(BCoreDeviceRemovedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BCoreDeviceRemovedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BCoreDeviceRemovedEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-device-removed-event/event-creation",
               BCoreDeviceRemovedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/barton-core-device-removed-event/property-access",
               BCoreDeviceRemovedEventTest,
               NULL,
               setup,
