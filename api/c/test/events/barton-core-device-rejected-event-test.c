...
typedef struct
{
    BCoreDeviceRejectedEvent *event;
} BCoreDeviceRejectedEventTest;

static void setup(BCoreDeviceRejectedEventTest *test, gconstpointer user_data)
{
    test->event = b_core_device_rejected_event_new();
}

// Teardown function called after each test
static void teardown(BCoreDeviceRejectedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BCoreDeviceRejectedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BCoreDeviceRejectedEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-device-rejected-event/event-creation",
               BCoreDeviceRejectedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/barton-core-device-rejected-event/property-access",
               BCoreDeviceRejectedEventTest,
               NULL,
               setup,
