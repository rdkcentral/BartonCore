...
typedef struct
{
    BDeviceServiceDeviceDiscoveredEvent *event;
} BDeviceServiceDeviceDiscoveredEventTest;

static void setup(BDeviceServiceDeviceDiscoveredEventTest *test, gconstpointer user_data)
{
    test->event = b_device_service_device_discovered_event_new();
}

// Teardown function called after each test
static void teardown(BDeviceServiceDeviceDiscoveredEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BDeviceServiceDeviceDiscoveredEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BDeviceServiceDeviceDiscoveredEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/device-service-device-discovered-event/event-creation",
               BDeviceServiceDeviceDiscoveredEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/device-service-device-discovered-event/property-access",
               BDeviceServiceDeviceDiscoveredEventTest,
               NULL,
               setup,
