...
typedef struct
{
    BCoreDiscoveryStartedEvent *event;
} BCoreDiscoveryStartedEventTest;

static void setup(BCoreDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    test->event = b_core_discovery_started_event_new();
}

// Teardown function called after each test
static void teardown(BCoreDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    g_clear_object(&test->event);
}

static void test_event_creation(BCoreDiscoveryStartedEventTest *test, gconstpointer user_data)
{
    g_assert_nonnull(test->event);
}

static void test_property_access(BCoreDiscoveryStartedEventTest *test, gconstpointer user_data)
{
...
int main(int argc, char *argv[])
{
    ...
    // Define test suite
    g_test_add("/barton-core-discovery-started-event/event-creation",
               BCoreDiscoveryStartedEventTest,
               NULL,
               setup,
               test_event_creation,
               teardown);
    g_test_add("/barton-core-discovery-started-event/property-access",
               BCoreDiscoveryStartedEventTest,
               NULL,
               setup,
