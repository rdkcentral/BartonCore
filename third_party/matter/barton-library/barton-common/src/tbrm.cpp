...
class FakeBorderRouterDelegate final : public ThreadBorderRouterManagement::Delegate
{
    ...
    {
        CopyCharSpanToMutableCharSpan("barton-br"_span, borderRouterName);
    }
...
EndpointId gThreadBorderRouterManagementServerEndpointId = UINT16_MAX;
void emberAfThreadBorderRouterManagementClusterInitCallback(EndpointId endpoint)
{
    if (gThreadBorderRouterManagementServer)
    {
        VerifyOrDie(endpoint == gThreadBorderRouterManagementServerEndpointId);
    }
    else
    {
        gThreadBorderRouterManagementServer.emplace(endpoint, &gBorderRouterDelegate, Server::GetInstance().GetFailSafeContext())
            .Init();
        gThreadBorderRouterManagementServerEndpointId = endpoint;
    }
}
