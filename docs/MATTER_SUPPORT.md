...
It is very likely clients of Barton may want to pick and choose their matter application configuration, or provide custom code. They would then need to generate their own custom Matter SDK target library and point Barton to it (see: `BDS_MATTER_LIB`). The Barton matter library build target thus also acts as a reference for clients to create their own Matter libraries.
