...
**`MQuickJsRuntime::ForceSnapshot()`** is a public synchronous function that acquires the JS mutex, calls `RecordHeapSnapshot` (which advances `sbmd.js.heap.peak_bytes` only if the current value exceeds the recorded peak), releases the mutex, and calls `TickleSampler()`. It SHALL silently no-op — without acquiring the JS mutex — if called before `MQuickJsRuntime::Initialize()` has succeeded (the metric handle null-check alone is insufficient because the JS context may not exist yet). Used in unit tests and available for on-demand telemetry. Any snapshot from any path resets the idle timer.
