...
- `takePicture` acts as a springboard symmetric with `stream`: it ignores `sessionId` and returns `{ "protocol": "onvif", "entryPoint": "/<deviceId>/ep/onvif/r/getSnapshotUrl" }`. Executing `getSnapshotUrl` performs an on-demand ONVIF `GetSnapshotUri` SOAP call and emits the resulting JPEG URL as a `snapshotUrl` event.
- Camera **credentials** (`username` as `RESOURCE_TYPE_USER_ID`, `password` as `RESOURCE_TYPE_PASSWORD`) are stored with the existing `RESOURCE_MODE_SENSITIVE` bit (encrypted at rest, redacted in logs). Configuration is **lazy**: `configureDevice` creates endpoints and credential resources but makes **no** authenticated ONVIF calls; authentication (WS-UsernameToken) happens on-demand at `getMediaUrl`/`getSnapshotUrl` time.
