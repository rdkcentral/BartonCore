...
The driver SHALL include a `fanOn` resource with type `boolean` as an optional resource derived from the Fan Control cluster (0x0202). The resource SHALL only be present when the device supports the Fan Control cluster, using a prerequisite gate on the `fanPercentCurrent` alias.
