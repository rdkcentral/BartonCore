...
- **`SbmdMapper` data model** gains `seedFromAttribute` (`std::optional<SbmdAttribute>`) and `seedFromScript` (string) fields. Presence of `seedFrom` is indicated by `seedFromAttribute.has_value()`.
...
- **`SpecBasedMatterDeviceDriver`** overrides `DoRegisterResources()` to read seed values from the attribute cache (via `MatterDevice::ReadSeedValueFromAttribute()`) and pass them directly to `createDeviceResource()`/`createEndpointResource()` — ensuring `DEVICE_ADDED` carries the correct initial value. It also overrides `DoSynchronizeDevice()` to call the shared `SeedInitialResourceValues()` helper (which uses `SeedResourceFromAttribute()` + `updateResource()`) at reconnect time.
...
- **Tests** (`core/test/src/sbmdParserTest.cpp`, `core/test/src/SbmdScriptTest.cpp`, `testing/test/door_lock_test.py`): Existing door-lock parser test and sideband integration tests updated; new parser, script, and integration tests added. `test_locked_resource_seeded_on_synchronize` sets the `matterLivenessTimeoutOverrideMs` device metadata property via `deviceServiceSetMetadata` to trigger immediate resubscription without waiting for the full negotiated liveness window (see Integration Test Infrastructure section). `DeviceDataCache::OnSubscriptionEstablished` is updated to reset `mLivenessTimeoutOverride` to `Clock::kZero` after each successful subscription, preventing a rapid-resubscription loop.
...
- **Non-goals**: Event replay from the `ClusterStateCache` event buffer for missed events during Barton downtime is explicitly out of scope. Surfacing `EventHeader` metadata (`mEventNumber`, `mTimestamp`, `mPriorityLevel`) in `sbmdEventArgs` or `updateResource` metadata is out of scope. Multi-instance cluster support is out of scope.
