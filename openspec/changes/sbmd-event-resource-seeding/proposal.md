...
- **Tests** (`core/test/src/sbmdParserTest.cpp`, `core/test/src/SbmdScriptTest.cpp`, `testing/test/door_lock_test.py`): Existing door-lock parser test and sideband integration tests updated; new parser, script, and integration tests added. `test_locked_resource_seeded_on_synchronize` uses `bartonMatterDeviceForceResubscription` (declared in `api/c/public/private/bartonMatterTestHelpers.h`) to trigger immediate resubscription without waiting for the full negotiated liveness window. `DeviceDataCache::OnSubscriptionEstablished` is updated to reset `mLivenessTimeoutOverride` to `Clock::kZero` after each successful subscription, preventing a rapid-resubscription loop when the override has been set by the test helper.
