...
- [x] 11.5 Add `barton.commFail.monitorIntervalSecs` property (`COMM_FAIL_MONITOR_INTERVAL_SECS_PROP` in `devicePrivateProperties.h`): applied via `deviceCommunicationWatchdogSetMonitorInterval()` in two ways: (a) one-shot read in `deviceService.c` immediately after `deviceServiceCommFailInit()` for values already persisted to the DB; (b) a `property-changed` signal handler (`OnCommFailMonitorIntervalPropertyChanged`) wired on the `BCorePropertyProvider` so that writes made after startup take effect immediately (handler also broadcasts `controlCond` to wake the watchdog thread without waiting for the current sleep to expire). Set to `"1"` in `test_locked_resource_seeded_on_synchronize` in `door_lock_test.py` after commission, inline in the test body. In production the property is absent and the 60 s default check interval is unchanged.
