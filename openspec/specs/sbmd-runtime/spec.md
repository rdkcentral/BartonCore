...
Specifies the Spec-Based Matter Driver (SBMD) runtime engine: how driver files are loaded and claimed, the v4 embedded-JavaScript execution model (two-pass evaluation, handler dispatch, result chains, deferred operations, alias resolution, and lifecycle), and the safety limits (per-invocation script timeouts, an overall operation timeout for deferred chains, and a maximum deferral depth) that bound execution.
