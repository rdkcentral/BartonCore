# onvif-camera-driver Specification

## Purpose
A native (non-SBMD) ONVIF/RTSP camera device driver for the abstract `camera` device class. It discovers cameras via ONVIF WS-Discovery, exposes the abstract `ep/camera` lifecycle plus a technology-specific `ep/onvif` endpoint, and resolves the RTSP media URL and snapshot URL on demand via authenticated ONVIF SOAP calls. It coexists with the Matter camera driver (ownership follows discovery), is gated behind the `BCORE_ONVIF` build flag, and uses an explicitly interim static per-device credential model.
