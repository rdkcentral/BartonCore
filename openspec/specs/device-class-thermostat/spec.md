...
Specifies the Barton `thermostat` device class: a device with a `thermostat`-profile endpoint exposing setpoint, local-temperature, system-mode, running-state, and optional fan-control resources. It also specifies how an SBMD thermostat driver maps the Matter Thermostat (0x0301) device type and its Thermostat (0x0201) and Fan Control (0x0202) clusters onto those resources. Concrete thermostat drivers are `.sbmd.js` files governed by the SBMD schema validator.
