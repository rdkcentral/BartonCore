...
This capability defines the SBMD runtime characterization test suite (`testing/observability/`): the scenarios that exercise the SBMD JS runtime at single-device, concurrent multi-device, heterogeneous multi-driver, and driver-count scale; the consolidated, graded metrics report those scenarios produce; and the DEBUG_GC build-flip tooling that makes production-representative runs reproducible on demand. The suite exists to answer, with data, whether SBMD's resource footprint scales with device/driver count and whether its behavior at scale exposes any implementation defect.
