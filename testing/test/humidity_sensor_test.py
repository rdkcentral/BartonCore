...
def test_commission_humidity_sensor(
    ...
    """Commission a humidity sensor and verify the humidity resource."""
    ...
    # Virtual device defaults: humidity = 5000 (50.00%) -> 50 whole percent
    hum_queue = resource_update_listener(client, "humidity")
    wait_for_resource_value(hum_queue, "50")


def test_humidity_sensor_update(
    default_environment, matter_humidity_sensor
):
    """Update humidity via side-band and verify Barton receives it."""
    commission_device(
        default_environment,
        matter_humidity_sensor,
        "environmentalSensor",
